...
def parse_args():
    parser = argparse.ArgumentParser(description="HiAD Inference")
    ...
    parser.add_argument("--checkpoint-root", default="results/dinomaly_checkpoints",
                        help="Path to a unified checkpoint root with current.json")
...
if __name__ == "__main__":
    ...
    if "preprocessing" not in loaded_config or "scoring" not in loaded_config:
        raise ValueError("Inference config requires preprocessing and scoring mappings")
    ...
    main_logger = create_logger("main", os.path.join(args.log_root, "inference.log"), print_console=True)
    main_logger.info(args)

    test_meta = read_jsonl_records(
        os.path.join(args.data_root, "test_uni.jsonl")
    )
    test_samples = [
        HRSample(
            image=os.path.join(args.data_root, sample["filename"]),
            mask=os.path.join(args.data_root, sample["mask"]) if sample.get("mask") else None,
            clsname=sample["clsname"],
            label=sample["label"],
            label_name=sample.get("label_name"),
        )
        for sample in test_meta
    ]

    inference_samples = [
        HRSample(
            image=sample.image.image_path,
            clsname=sample.clsname,
        )
    ...
    main_logger.info(f"Inference done. Results logged to {args.log_root}")
