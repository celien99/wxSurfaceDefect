...
SurfaceMind 将预训练 DINOv3 表征、官方式 Dinomaly2 语义重建、原图坐标切片和正常样本阈值校准组合为一条可训练、可推理、可评估的工程链路。它适合缺陷样本稀少、缺陷形态开放、且需要保留微小缺陷空间证据的场景。
...
- **局部细节、复核与全局语义协同**：动态补丁分支检测局部纹理，候选区域经微补丁复核；整图缩略前向既产出教师特征去全局上下文的 recenter 锚，也作为候选路由的全局先验。
...
flowchart LR
    ...
    subgraph MODEL[语义重建 Dinomaly2]
        D --> G[冻结 DINOv3 教师\n中间层完整 token 序列]
        R --> G
        E --> G
        E --> H[整图缩略前向]
        H --> I[按组全局锚 recenter + LN]
        G --> I
        I --> J[两段式窄噪声瓶颈 +\n线性注意力解码器]
        J --> K[语义余弦距离重建残差]
    end
...
| 上下文能力 | 主要依赖人工构造的局部算子 | 受训练任务与记忆特征限制 | 局部补丁与整图缩略（recenter 锚 + 路由先验）共同建模 |
...
| `hiad/models/dinov3.py` | 表征底座 | 通过 `timm` 加载预训练 DINOv3，固定全部编码器参数并提取中间层 `[2..9]` 的完整 token 序列（含 cls/register 前缀） |
| `hiad/detectors/hr_dinomaly.py` | 异常检测器 | 官方式 Dinomaly2 语义重建：两段窄噪声瓶颈 + 线性注意力解码器在整序列上重构教师，教师按组减整图缩略全局锚并 LayerNorm，输出语义余弦距离异常证据 |
...
2. **原图坐标采样与复核**：`DynamicTaskGenerator` 将原图平铺为边界对齐的主补丁；`stride` 可设置重叠，以增加边界区域的冗余观测。粗扫候选区域由微补丁复核，缩略图任务把整图缩到固定边长。
3. **特征重建与去全局 recenter**：冻结的 DINOv3 输出中间层 `[2..9]` 的完整 token 序列（含 cls 与 register 前缀）；序列经两段式窄噪声瓶颈与 8 层线性注意力解码器在整序列上重构。教师目标按层分组减『该源图整图缩略前向的全局锚』再 `LayerNorm`（context-aware recentering），使异常证据聚焦局部偏离而非整图亮度/光照。训练只更新各任务的瓶颈与解码器，教师完全冻结。
4. **困难正常样本关注**：余弦蒸馏损失支持从 warmup 逐步提高 hard-mining 比例，并以较低系数保留易样本梯度；同时提供梯度裁剪、AMP 与 TF32 开关。训练以逐 epoch 均值损失平台期早停，避免在迭代预算上无谓消耗。
5. **异常图生成**：对每组编码/重建 token 计算非负余弦距离图（clamp 到 `[0,2]`），在组间逐位置取最大值生成 token 异常图，再把每组图分别上采样到补丁尺寸并在组间取最大；异常证据为纯语义余弦距离（semantic-only），不叠加记忆库或高频分支。不同补丁以 Hann 权重回填，减轻拼接边缘不连续。
6. **证据唯一性**：异常证据仅为逐组语义余弦距离的最大值；曾尝试的记忆库、高频与多证据融合在真实数据上回退，已被移除，打分与训练目标不分裂。图像级分数由最终异常图 Top-K 与缩略图 token 分数构成。
...
以下命令使用 512 x 512 主补丁与 512 x 512 整图缩略任务（整图缩略前向同时产出全局 recenter 锚与候选路由先验）：
...
| `--thumbnail-size` | 整图缩略任务输入边长，必须为 16 的倍数；默认应与 `ANCHOR_CANVAS`（512）一致，作为全局 recenter 锚画布 |
...
默认配置在 [`configs/dinomaly.yaml`](configs/dinomaly.yaml)，所有字段均为必填项。训练与推理共享这一份配置映射；补丁尺寸、缩略图尺寸与复核参数来自任务定义，不再复制三份同内容配置。配置包含训练迭代预算（默认 2000）、窄噪声瓶颈 dropout、困难样本比例与 warmup、混合精度与 TF32、每源图像采样补丁数、Top-K 图像评分、`global_routing_weight`、`refinement_bridge_gap_tiles`、图像/像素/组件校准分位数和质量检查阈值。
...
每个 `*_weight.pkl` 只保存对应任务的两段式窄噪声瓶颈与线性注意力解码器状态（粗扫、复核、缩略图各一份，独立训练与推理）；DINOv3 教师冻结不落盘。`score_calibration.json` 保存图像、像素和组件阈值。加载入口只要求当前任务文件存在，并按当前格式直接读取，不提供旧格式兼容或额外架构校验；架构或配置改变后直接重新训练并校准，不复用旧训练产物。
