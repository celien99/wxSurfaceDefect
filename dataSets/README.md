...
```
    |--data
        ...
        |--create_realiad2k.py
```  
Run `python create_realiad2k.py` to create the dataset. The HiAD runners use the
root-level `train_uni.jsonl` and `test_uni.jsonl` files. Every normal
`train_uni.jsonl` record participates in shared Dinomaly training, then the same
complete set establishes one global calibration threshold. Records retain their
`clsname`; it selects only the matching foreground reference bundle.

No validation split is required or created by the current runtime. An
independent validation set can be added later without changing the unified JSONL
contract, CLI, shared checkpoint topology, or global threshold design.
